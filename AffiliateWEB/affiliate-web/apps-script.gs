...
const HEADERS = {
  ...
  '🔔 Reminders': ['ID', 'ข้อความ', 'ประเภท', 'ค่าวันที่', 'เปิดใช้งาน'],
};
...
function doPostInner(e) {
  ...
  if (p.action === 'add') {
    ...
    const rowIdx = values.findIndex(function (r) { return String(r[0]) === String(p.key); });
    if (rowIdx === -1) return json({ ok: false, error: 'NOT_FOUND' });
    sheet.deleteRow(rowIdx + 2); // ลบแถวแรกที่ตรง key เท่านั้น — ถ้ามีซ้ำ กดอีกครั้งเพื่อลบตัวถัดไป
  ...
  } else if (p.action === 'addNewProductFull') {
    return addNewProductFull(sheet, p);
  } else if (p.action === 'setCommissionDay') {
    return setCommissionDay(sheet, p);
  } else {
...
// ---- เพิ่มสินค้าใหม่ครบวงจร: เขียน 4 ชีตในคราวเดียว (สินค้า, ⚡️Progress, 🗂️ ListITME, และคำอธิบาย/ShopeeLink ถ้ากรอกมา) ----
// เรียกผ่าน POST { sheet: 'สินค้า', action: 'addNewProductFull', sku, name, category, commissionPct, tier: 'B'|'C', description, shopeeLinks: [...] }
// sheet ที่ dispatcher หาให้คือชีต "สินค้า" อยู่แล้ว (ใช้เติมแถวแรกตรงๆ) ส่วนอีก 3-4 ชีตหาเพิ่มเองข้างในฟังก์ชันนี้
// สูตร dropdown "EP.0"–"EP.100" ใช้กับทั้ง ⚡️Progress และ 🗂️ ListITME ตอนเพิ่มแถวใหม่ (appendRow/insertRowAfter ไม่ก็อป data validation ของแถวข้างเคียงมาให้อัตโนมัติ)
function epDropdownRule() {
  const values = [];
  for (let n = 0; n <= 100; n++) values.push('EP.' + n);
  return SpreadsheetApp.newDataValidation().requireValueInList(values, true).setAllowInvalid(false).build();
}

// ก็อปหน้าตา (สี/ขอบ/รูปแบบ) ของเซลล์ dropdown จากแถวก่อนหน้ามาทับ โดยไม่แตะค่าที่ตั้งไว้แล้วในเซลล์ปลายทางเลย
function copyCellFormat(sheet, fromRow, toRow, col) {
  sheet.getRange(fromRow, col).copyTo(sheet.getRange(toRow, col), SpreadsheetApp.CopyPasteType.PASTE_FORMAT, false);
}

function addNewProductFull(sheet, p) {
  const headers = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
  const productData = {
    'SKU': p.sku, 'ชื่อสินค้า': p.name, 'หมวดหมู่': p.category,
    'ราคา': '', 'ค่าคอม %': p.commissionPct || '', 'Link สินค้า': '', 'จำนวน EP': 0, 'สถานะ': 'กำลังขาย',
  };
  sheet.appendRow(headers.map(function (h) { return productData[h] !== undefined ? productData[h] : ''; }));

  const itmLabel = p.sku + ' ' + p.name;
  ...
  const progSheet = SS.getSheetByName(PROGRESS_SHEET);
  if (progSheet) {
    progSheet.appendRow([itmLabel, 'EP.0', 'EP.0', 'EP.0', 'EP.0', 'EP.0']);
    const progRow = progSheet.getLastRow();
    Object.keys(PROGRESS_COLS).forEach(function (stage) {
      const col = PROGRESS_COLS[stage];
      progSheet.getRange(progRow, col).setDataValidation(rule);
      if (progRow > 2) copyCellFormat(progSheet, progRow - 1, progRow, col); // หน้าตา dropdown ก็อปจากแถว ITM ก่อนหน้า ค่า EP.0 ที่ตั้งไว้ไม่เปลี่ยน
    });
  }

  // 🗂️ ListITME — แทรกต่อท้ายบล็อก Tier ที่เลือก (B/Bต/C/Cต แบบตรงตัวเท่านั้น ไม่รวม B2/A) Footage/Draft เริ่มที่ EP.0 พร้อม dropdown
  const listSheet = SS.getSheetByName(LISTITME_SHEET);
  const VALID_NEW_TIERS = ['B', 'Bต', 'C', 'Cต'];
  if (listSheet && VALID_NEW_TIERS.indexOf(p.tier) !== -1) {
    const values = listSheet.getDataRange().getValues();
    let lastMatchRow = 0;
    for (let r = 0; r < values.length; r++) {
      if (String(values[r][LISTITME_TIER_COL - 1] || '').trim() === p.tier) lastMatchRow = r + 1;
    }
    const insertAt = lastMatchRow || listSheet.getLastRow();
    listSheet.insertRowAfter(insertAt);
    const newRow = insertAt + 1;
    listSheet.getRange(newRow, LISTITME_NAME_COL).setValue(itmLabel);
    listSheet.getRange(newRow, LISTITME_EP_COL).setValue(0);
    listSheet.getRange(newRow, LISTITME_TIER_COL).setValue(p.tier);
    listSheet.getRange(newRow, LISTITME_STAGE_COLS.footage).setValue('EP.0').setDataValidation(rule);
    listSheet.getRange(newRow, LISTITME_STAGE_COLS.draft).setValue('EP.0').setDataValidation(rule);
    // หน้าตา dropdown ก็อปจากแถว ITM ก่อนหน้า (แถวที่แทรกต่อจาก insertAt) ค่า EP.0 ที่ตั้งไว้ไม่เปลี่ยน
    copyCellFormat(listSheet, insertAt, newRow, LISTITME_STAGE_COLS.footage);
    copyCellFormat(listSheet, insertAt, newRow, LISTITME_STAGE_COLS.draft);
    // สูตร Footage/Draft เหลือ ให้ตรงกับที่ fillListItmeRemaining ใส่ไว้ในคอลัมน์ G/H
    listSheet.getRange(newRow, 7).setFormula('=IFERROR(VALUE(SUBSTITUTE(E' + newRow + ',"EP.",""))-C' + newRow + ',"")');
    listSheet.getRange(newRow, 8).setFormula('=IFERROR(VALUE(SUBSTITUTE(F' + newRow + ',"EP.",""))-C' + newRow + ',"")');
  }

  // 🍊🔗ShopeeLink — สร้างแถวเสมอ โครงสร้างจริง: A=ชื่อ, B=ประเภท(dropdown เดิม), C=ค่าคอม%, D+=ลิงก์ (สูงสุด 5 ถ้ามี)
  // ใช้หมวดหมู่ที่พี่เลือกในฟอร์มตรงๆ (p.category) ไม่เดาใหม่ ให้ตรงกับ dropdown ที่มีอยู่แล้วในชีตเป๊ะๆ
  const links = (p.shopeeLinks || []).filter(function (u) { return u; }).slice(0, 5);
  const linkSheet = SS.getSheetByName('🍊🔗ShopeeLink');
  if (linkSheet) {
    linkSheet.appendRow([itmLabel, p.category, p.commissionPct || ''].concat(links));
    const newRow = linkSheet.getLastRow();
    // ก็อป data validation (dropdown ประเภทสินค้า) + หน้าตา (สี) จากแถวก่อนหน้ามาใช้เลย ไม่ต้องเดา list เอง ให้ตรงกับของจริงในชีตเป๊ะๆ ค่าที่ตั้งไว้ไม่เปลี่ยน
    if (newRow > 2) {
      const prevRule = linkSheet.getRange(newRow - 1, 2).getDataValidation();
      if (prevRule) linkSheet.getRange(newRow, 2).setDataValidation(prevRule);
      copyCellFormat(linkSheet, newRow - 1, newRow, 2);
    }
  }
...
// ============================================================
// บันทึกค่าคอมรายวัน — แท็บ "💰 บันทึกค่าคอม" กรอกสรุป TikTok/Shopee ลงชีต "7️⃣ 📝  บันทึกค่าคอม-<เดือน>"
// (ชื่อชีตเปลี่ยนทุกเดือนเหมือน TODOLIST — ฝั่งเว็บส่ง sheet มาเองในทุก request คอลัมน์ A ของชีตนี้เป็นเลขวันที่ 1-31 ตรงๆ ไม่ใช่ Date object)
// ============================================================
const COMMISSION_COLS = {
  tiktokSold: 2, tiktokCommission: 3, tiktokWithdraw: 4, tiktokSettledQty: 7, tiktokSettledAmt: 8, // B,C,D,G,H
  shopeeWithdraw: 12, shopeeAdsSp: 13, shopeeAdsFb: 14, shopeeSettled: 15, // L,M,N,O
  shopeeVdoQty: 23, shopeeVdoGmv: 24, shopeeVdoComm: 25, shopeeVdoAds: 26, // W,X,Y,Z
  linkAds: 31, // AE
  linkRuamQty: 33, linkRuamGmv: 34, linkRuamComm: 35, // AG,AH,AI
  linkExtraQty: 36, linkExtraGmv: 37, linkExtraComm: 38, // AJ,AK,AL
};

function findCommissionDayRow(sheet, day) {
  const lastRow = sheet.getLastRow();
  const values = sheet.getRange(1, 1, lastRow, 1).getValues();
  for (let r = 0; r < values.length; r++) {
    const v = parseInt(values[r][0], 10);
    if (!isNaN(v) && v === day) return r + 1;
  }
  return 0;
}

// p: { day: 1-31, tiktokSold, tiktokCommission, ... (คีย์ตาม COMMISSION_COLS) } — เขียนเฉพาะช่องที่ส่งค่ามา (ไม่ทับช่องที่เว้นว่างไว้)
function setCommissionDay(sheet, p) {
  const day = parseInt(p.day, 10);
  if (isNaN(day) || day < 1 || day > 31) return json({ ok: false, error: 'INVALID_DAY' });
  const rowNum = findCommissionDayRow(sheet, day);
  if (!rowNum) return json({ ok: false, error: 'DAY_ROW_NOT_FOUND' });
  Object.keys(COMMISSION_COLS).forEach(function (key) {
    if (p[key] === undefined || p[key] === '' || p[key] === null) return;
    sheet.getRange(rowNum, COMMISSION_COLS[key]).setValue(Number(p[key]));
  });
  return json({ ok: true });
}
